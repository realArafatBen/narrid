...
class Category extends StatelessWidget {
  final String id, cat_name, image;
  Category({Key key, @required this.id, @required this.cat_name, this.image})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context) => ProductCategoriesBloc(ProductCategoriesRepos())
          ..add(CatVStarted(
            id,
            brandId,
            productType,
              ...
              children: [
                ...
                BlocBuilder<ProductCategoriesBloc, ProductCategoriesState>(
                  builder: (context, state) {
                    if (state is CatVLoading) {
                      return Center(
                        child: CircularProgressIndicator(),
                      );
                    } else if (state is CatVLoaded) {
                      return ProductWidget(state: state, context: context);
                    } else if (state is CatVErrorLoading) {
                      return Center(
                        child: CircularProgressIndicator(),
                      );
                    } else {
