...
class CheckoutFoodBloc
    extends Bloc<CheckoutProductEvent, CheckoutProductState> {
  final FoodProductRep foodProductRep;

  CheckoutFoodBloc({@required this.foodProductRep})
  ...
  Stream<CheckoutProductState> mapEventToState(
    ...
    if (event is CheckoutProductStarted) {
      try {
        final List<FoodCartHelper> cart = await foodProductRep.getProducts();
